...
class Validate {
    ...
    private function doValidation() {
        ...
        if(!array_key_exists($F['Name'], $this->Data) || empty($this->Data[$F['Name']])) {
            if($F['Required']) {
    ...
    private function err($Error) {
        $F = $this->Field;
        if(!$this->DebugMode) {
            // Show the original, less descriptive error
            $Error = $F['Error'];
        }
        $this->Errors[] = $Error;
        return $Error;
    ...
    private function debugParameters($P, $FN) {
        ...
        foreach($P as $ParamKey=>$V) {
            if(!array_key_exists($ParamKey,$this->AvailableParameters)) {
